create or replace function public.sauti_policy_numeric(
  setting_key text,
  fallback numeric
)
returns numeric
...
as $$
  select coalesce(
    (
      select nullif(value ->> setting_key, '')::numeric
...
as $$
declare
  savings_threshold numeric := public.sauti_policy_numeric('mandatorySavingsThreshold', 5000);
  shares_threshold numeric := public.sauti_policy_numeric('mandatorySharesThreshold', 3000);
  share_price numeric := 100;
  share_basket numeric;
begin
  new.savings_balance := coalesce(new.savings_balance, 0);
  new.shares := coalesce(new.shares, 0);
  new.share_reserve_balance := coalesce(new.share_reserve_balance, 0);

  if new.savings_balance < 0 then
    raise exception 'Daily compliance contribution cannot be negative.';
  end if;

  if new.shares < 0 or new.share_reserve_balance < 0 then
    raise exception 'Share balances cannot be negative.';
  end if;

  if new.savings_balance > savings_threshold then
    raise exception 'Daily compliance contribution % exceeds mandatory threshold %.',
      new.savings_balance,
      savings_threshold;
  end if;

  share_basket := (new.shares * share_price) + new.share_reserve_balance;
  if share_basket > shares_threshold then
    raise exception 'Share basket % exceeds mandatory threshold %.',
      share_basket,
      shares_threshold;
  end if;
...
before insert or update of savings_balance, shares, share_reserve_balance
on public.members
for each row
execute function public.tg_enforce_member_financial_invariants();
