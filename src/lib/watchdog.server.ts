...
export async function runWatchdogAnalysis(question: string) {
  const supabaseAdmin = getSupabaseAdminOrNull();
  if (!supabaseAdmin) {
    const missing = getSupabaseAdminEnvStatus().missing.join(", ");
    throw new Error(
      `Watchdog analysis is unavailable until the server has: ${missing}. Add those values to local env or hosting secrets.`,
    );
  }
...
Your job: detect anything suspicious and answer the director's question.
Look for: rank/role elevation attempts, deleted or modified transactions, repeated failed logins, unusual money outflows, members with negative or wildly inconsistent balances, loans whose paid > principal, staff editing their own records, secrets being added or removed at odd hours, duplicated audit entries (potential replay), and patterns that do not match normal SACCO operations.
Respond in markdown with sections: Issues detected, Notable activity, Looks fine. Be specific - quote actor names, ids, amounts (KSh), and timestamps. If nothing is off, say so. Never claim you took an action - you cannot.`;
