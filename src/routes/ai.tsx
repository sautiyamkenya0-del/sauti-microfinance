...
function AiPage() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    {
      role: "assistant",
      content:
        "Hi — I'm **SautiAI**. Ask me about a member, loan, or transaction. I can also flag anomalies and propose actions for you to confirm.",
    },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      if (!r.ok || !r.body) {
        ...
        setMsgs((p) =>
          p.map((m, i) => (i === p.length - 1 ? { ...m, content: `⚠️ ${err.error}` } : m)),
        );
        setBusy(false);
        return;
      }
  ...
  return (
    <>
      <AppHeader
        title="SautiAI"
        subtitle="Read-only assistant with action proposals — confirm before anything writes."
      />
      <main className="flex-1 p-6 lg:p-8 flex flex-col gap-4 max-w-4xl w-full mx-auto">
        ...
        >
          <input
            ...
            placeholder="Summarize SBC003 · flag any overdue loans · propose disbursement for L0008…"
